...
vector<Problem> problems = {
    ...
    {"Linux 系统中编译 C 程序的编译器是", {{"gcc", true}, {"g++"}, {"fpc"}, {"gdb"}}},
    {"Linux 系统中编译 Pascal 程序的编译器是", {{"fpc", true}, {"gcc"}, {"g++"}, {"gdb"}}},
    ...
    {"使用 gcc 编译 C 程序时，生成调试信息的命令行选项是", {{"-g", true}, {"-o"}, {"-c"}, {"-d"}}},
    {"使用 gcc 编译 C 程序时，生成所有警告信息的命令行选项是", {{"-Wall", true}, {"-Werror"}, {"-g"}, {"-o"}}},
    {"使用 gcc 编译 C 程序时，只编译生成目标文件的命令行选项是", {{"-c", true}, {"-o"}, {"-g"}, {"-d"}}},
    {"使用 gcc 编译 C 程序时，指定输出文件名的命令行选项是", {{"-o", true}, {"-c"}, {"-g"}, {"-d"}}},
    {"如果 C 程序中使用了 math.h 中的函数，在编译时需要加入选项", {{"-lm", true}, {"-lmath"}, {"-m"}, {"-math"}}},
    ...
    {"源代码文件 my.c 创建一个备份 myc.bak 时，使用的命令是", {{"cp my.c myc.bak", true}, {"cp myc.bak my.c"}, {"mv my.c myc.bak"}, {"mv myc.bak my.c"}}},
    ...
    {"Pascal 中 integer 和 long integer 类型的长度和编译选项是否有关系", {{"有关系", true}, {"没关系"}}},
    {"NOI 考试对 C++ 语言模板的使用有限制吗？", {{"没有", true}, {"有"}}},
    {"NOI 考试对 PASCAL 语言的使用有限制吗？", {{"有", true}, {"没有"}}},
    ...
    {"在 NOI 考试中，Pascal 源文件的扩展名规定为", {{"pas", true}, {"cpp"}, {"c"}, {"txt"}}},
    {"在 NOI 考试中，C 源文件的扩展名规定为", {{"c", true}, {"cpp"}, {"pas"}, {"txt"}}},
    ...
    {"复评结束后是否还能提交复评申请", {{"不能", true}, {"能"}, {"也许能"}, {"也许不能"}}},
    ...
    {"考试中选手自行重新启动机器，能否获得加时？", {{"不能", true}, {"能"}, {"也许能"}, {"也许不能"}}},
    ...
    {"在 NOI 上机考试中，允许选手使用的编程语言包括", {{"C", true}, {"C++", true}, {"Pascal", true}, {"Python"}}},
    ...
    {"没有自行复测结果（结果正确性、所用时间、所用空间等）支持的申诉是否会被受理", {{"否", true}, {"是"}}},
    ...
    {"与十六进制数（AB）等值的二进数是", {{"10101011", true}, {"10101111"}, {"10000000"}, {"10100011"}}},
    {"Linux 中查看当前路径使用的命令是", {{"pwd", true}, {"ls"}, {"cat"}, {"ps"}}},
    {"在 Linux 下建立目录使用的命令是", {{"mkdir", true}, {"ls"}, {"rmdir"}, {"cat"}}},
    {"NOI 比赛中提供的 Pascal IDE 环境有", {{"GUIDE", true}, {"Lazarus", true}, {"Anjuta"}, {"vim"}}},
